...
ensure_dm_can_interface() {
  ...
  local -a privilege=()
  [[ $bitrate =~ ^[0-9]+$ && $bitrate -gt 0 ]] || { echo "DM_CAN_BITRATE must be a positive integer." >&2; return 2; }
  [[ $restart_ms =~ ^[0-9]+$ ]] || { echo "DM_CAN_RESTART_MS must be a non-negative integer." >&2; return 2; }
...
usage() {
  ...
  echo "Disable selected DM motors, clear their faults, and verify feedback."
}

set_selection() {
  [[ -z $SELECTION_MODE ]] || { echo "Choose exactly one of --joint, --joints, or --all." >&2; exit 2; }
  SELECTION_MODE=$1
  SELECTION_VALUE=${2:-}
}
...
[[ -n $SELECTION_MODE ]] || { echo "One joint selection is required." >&2; usage >&2; exit 2; }
[[ $INTERFACE =~ ^[[:alnum:]_.-]+$ ]] || { echo "Invalid SocketCAN interface: $INTERFACE" >&2; exit 2; }

normalize_joint() {
  local value=${1,,}
  value=${value#joint}
  [[ $value =~ ^[1-6]$ ]] || { echo "Invalid joint '$1'; expected joint1..joint6 or 1..6." >&2; exit 2; }
  printf '%s\n' "$value"
}
...
case $SELECTION_MODE in
  ...
  joints)
    ...
    for value in "${VALUES[@]}"; do JOINT_IDS+=("$(normalize_joint "$value")"); done
    ;;
...
COMMAND=(ros2 run rebotarm_dm_motor_sdk dm_motor_clear_fault --execute "${INTERFACE}" "${JOINT_IDS[@]}")
if [[ $DRY_RUN == true ]]; then
  printf 'Dry run; no CAN command was sent: '; printf '%q ' "${COMMAND[@]}"; printf '\n'
  exit 0
fi
ensure_dm_can_interface "$INTERFACE"
read -r -p "Type DM_CLEAR to continue: " confirmation
[[ $confirmation == DM_CLEAR ]] || { echo "Cancelled."; exit 1; }
