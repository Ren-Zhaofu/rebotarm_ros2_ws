...
ensure_dm_can_interface() {
  ...
  [[ $bitrate =~ ^[0-9]+$ && $bitrate -gt 0 ]] || {
    echo "DM_CAN_BITRATE must be a positive integer." >&2; return 2;
  }
  [[ $restart_ms =~ ^[0-9]+$ ]] || {
    echo "DM_CAN_RESTART_MS must be a non-negative integer." >&2; return 2;
  }
...
usage() {
  ...
  echo "Set the current mechanical position as zero for selected DM motors."
}
...
COMMAND=(ros2 run rebotarm_dm_motor_sdk dm_motor_set_zero --execute "${INTERFACE}" "${JOINT_IDS[@]}")
...
read -r -p "Type DM_ZERO to continue: " confirmation
[[ $confirmation == DM_ZERO ]] || { echo "Cancelled."; exit 1; }
