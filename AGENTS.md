# 仓库协作规范

## 项目范围

- 本仓库是 ROS 2 Humble 工作空间。
- ROS 功能包统一放在 `src/`，工作空间级辅助脚本统一放在 `scripts/`。
- 不要将 `build/`、`install/` 或 `log/` 中的生成文件加入 Git。
- 工作区存在未提交修改时，保留与当前任务无关的用户改动。

## 编译与验证

- 使用 `./scripts/build.sh` 编译整个工作空间。
- 对局部修改，优先只编译受影响的功能包：
  `./scripts/build.sh --packages-select <package_name>`
- 提交前执行适合被修改功能包的检查。
- 如果有检查无法执行，需要说明具体原因。

## 修改边界

- 每次修改只聚焦一个主题。
- 不要混合机器人描述、控制、运动规划和硬件驱动的职责。
- 模块专属规则放在对应功能包内的 `AGENTS.md` 中。
- 不要删除、覆盖或格式化与当前任务无关的文件。

## 电机脚本对等规则

- `scripts/motor/dm/` 和 `scripts/motor/rs/` 中面向相同实机操作的脚本应成对提供；
  任一侧新增读取、清故障、校零、回零等能力时，必须同步检查并补齐另一侧。
- 成对脚本应尽量保持一致的文件命名、关节选择参数、`--interface`、`--dry-run`、
  交互确认和执行结果语义；协议差异或底层工具限制必须在脚本帮助或文档中说明。
- 不得仅复制外层脚本而绕过协议差异；DM 和 RS 各自所需的 SDK 可执行程序、状态验证
  与失能保护必须在对应功能包内实现和测试。

## Git 规范

- 铁律：凡是代理完成了文件修改，必须在必要验证通过后执行 `git add .`，再执行
  对应的 `git commit -m "类型(模块): 中文修改说明"`，无需等待用户再次提醒。
- 执行 `git add .` 前必须检查工作区；如有与当前任务无关的用户改动，不得将其纳入
  本次提交。
- 创建提交前先查看近期 `git log`，沿用仓库现有提交信息风格；当前格式为
  `类型(模块): 中文修改说明`。
- 提交信息统一使用 `类型(模块): 中文修改说明`。
- 常用类型包括 `feat`、`fix`、`refactor`、`docs`、`test`、`build` 和 `chore`。
- 一次提交只包含一个主题。
- 自动提交时不要包含与当前任务无关的用户改动。
- 不要提交编译产物、日志、缓存、密钥或本机专用配置。
- 除非用户明确要求，否则不要推送提交。

## ROS 规范

- 在 `package.xml` 和功能包的构建配置中声明依赖。
- 资源路径优先使用相对于功能包的路径，不要使用本机绝对路径。
- 启动文件、配置、URDF/Xacro 和源代码应放在功能包的标准目录中。
- CAD 导出的动力学参数、限制和碰撞几何，在依据硬件规格核验前均视为未验证数据。
