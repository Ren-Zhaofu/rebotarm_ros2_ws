...
void print_states(
    ...
    const std::array<bool, kMotorCount> &mode_seen, bool clear_screen) {
  if (clear_screen) {
...
int main(int argc, char **argv) {
  ...
  while (reporting_ok && g_running &&
    ...
    if (watch && complete && now >= next_print) {
      print_states(states, seen, run_modes, mode_seen, printed);
      printed = true;
  ...
  if (!watch || !printed) {
    print_states(states, seen, run_modes, mode_seen, false);
  }
  for (const bool value : seen) {
