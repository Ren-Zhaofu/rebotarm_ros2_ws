...
std::vector<std::size_t> parse_indices(int argc, char **argv) {
  ...
  for (int i = 3; i < argc; ++i) {
    ...
    if (consumed != std::string(argv[i]).size() || id < 1 ||
        id > kMotorCount) {
...
int main(int argc, char **argv) {
  ...
  for (const auto index : selected) {
    if (!bus.set_active_report(index, true)) {
      std::cerr << "joint" << index + 1 << " feedback: "
                << bus.last_error() << '\n';
      cleanup();
      return 1;
    }
  ...
  if (!collect_selected(bus, selected, before, std::chrono::milliseconds(500))) {
    std::cerr << "Not all selected motors returned feedback; zero was not written\n";
    cleanup();
    return 1;
  }
  for (const auto index : selected) {
    if (before[index].fault != 0) {
      std::cerr << "joint" << index + 1 << " fault=0x" << std::hex
                << static_cast<unsigned int>(before[index].fault) << std::dec
  ...
  for (const auto index : selected) {
    if (!bus.disable(index)) {
      std::cerr << "joint" << index + 1 << " disable: "
                << bus.last_error() << '\n';
      cleanup();
      return 1;
    }
  ...
  for (const auto index : selected) {
    if (!bus.set_active_report(index, false)) {
      std::cerr << "joint" << index + 1 << " stop feedback: "
                << bus.last_error() << '\n';
      cleanup();
      return 1;
    }
  ...
  for (const auto index : selected) {
    if (!bus.set_zero(index)) {
      std::cerr << "joint" << index + 1 << " set zero: "
                << bus.last_error() << '\n';
      cleanup();
      return 1;
    }
  ...
  for (const auto index : selected) {
    if (!bus.set_active_report(index, true)) {
      std::cerr << "joint" << index + 1 << " verification feedback: "
                << bus.last_error() << '\n';
      cleanup();
      return 1;
    }
  ...
  if (!collect_selected(bus, selected, after, std::chrono::milliseconds(500))) {
  ...
  bool success = true;
  for (const auto index : selected) {
    const auto &old_state = before[index];
    const auto &new_state = after[index];
    std::cout << "joint" << index + 1 << ": " << old_state.position
              << " rad -> " << new_state.position << " rad";
    if (std::abs(new_state.position) > kZeroTolerance || new_state.fault != 0) {
